...
class TimingReportTemplate:
    ...
    def _expand_rows(self, templates: list[dict]) -> list[dict]:
        out: list[dict] = []
        for t in templates:
    ...
    def _generate_segment_points(self, segment_rows: list[dict], segment_name: str, path_ctx: dict[str, Any], point_gen: dict[str, Any]) -> list[str]:
        pin_count = sum(1 for r in segment_rows if (r.get("type") or "").strip().lower() == "pin")
        out: list[str] = []
    ...
    def _build_path_ctx(self, config: dict, path_index: int, seed: int | None, launch_rows: list[dict], capture_rows: list[dict]) -> dict[str, Any]:
        ...
        # start/end from launch pin rows
        pin_indices = [i for i, r in enumerate(launch_rows) if (r.get("type") or "").strip().lower() == "pin"]
        if pin_indices and launch_points:
            ctx["startpoint"] = launch_points[pin_indices[0]]
            ctx["endpoint"] = launch_points[pin_indices[-1]]
        else:
    ...
    def generate(self, config: dict, output_path: str, seed: int | None = None) -> None:
        ...
        launch_rows = self._expand_rows(plan.row_templates)
        capture_rows = self._expand_rows(plan.capture_templates) if plan.capture_templates else []
        ...
        for path_idx in range(num_paths):
            ...
            if plan.separator and capture_rows:
                lines.append(plan.separator)

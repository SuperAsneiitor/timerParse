"""
对比两个 path_summary CSV（golden vs test），按 path_id 对齐，
对 arrival_time、required_time、slack 计算 (test - golden) / golden * 100%，输出对比结果。
"""
    ...
    for col in RATIO_COLUMNS:
        ...
        if vals:
            ...
            col_stats = {
                ...
                "std": None, "p90": None, "p95": None, "p99": None,
...
def write_stats_csv(stats: dict, path: Path) -> None:
    ...
    fieldnames = [
        ...
        "p90", "p95", "p99", "threshold_value", "threshold_count", "threshold_ratio",
    ]
...
def generate_html_report(
    ...
    for pair, data in stats.get("correlations", {}).items():
        rows_corr.append(f"<tr><td>{pair}</td><td>{data.get('count')}</td><td>{data.get('pearson')}</td></tr>")
    chart_tags = []
...
def run_compare(args) -> int:
    ...
    charts_dir = Path(args.charts_dir) if getattr(args, "charts_dir", "").strip() else (out_path.parent / "charts")
    chart_files = {}

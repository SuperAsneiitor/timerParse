...
- parser/   Timing 解析唯一实现：TimeParser 子类 + YAML 布局引擎 TimingParserV2
- extract.py   主抽取：单进程或多进程 Pool → 标准 CSV
- parser.parallel_extract  高吞吐：分割器 + 队列 + Worker（解析器与 extract 相同）
...
from .parser.format1_parser import Format1Parser
from .parser.format2_parser import Format2Parser
from .parser.pt_parser import PtParser

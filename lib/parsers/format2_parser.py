...
class Format2Parser(TimeParser):
    ...
    def _parseInputPin(self, raw: dict[str, str], content: str, col_pos: dict[str, int]) -> tuple[str, dict[str, str]]:
        ...
        attrs["Trans"] = raw_trans if self._isReasonableNum(raw_trans, 1.0) else trans
        attrs["Type"] = raw.get("Type", "")
        raw_ddelay = (raw.get("D-Delay") or "").strip()
        raw_delay = (raw.get("Delay") or "").strip()
        raw_time = (raw.get("Time") or "").strip()
        attrs["D-Delay"] = raw_ddelay if self._isReasonableNum(raw_ddelay, 2.0) else d_delay
        attrs["Delay"] = raw_delay if self._isReasonableNum(raw_delay, 10.0) else delay
        attrs["Time"] = raw_time if self._isReasonableNum(raw_time, 20.0) else time

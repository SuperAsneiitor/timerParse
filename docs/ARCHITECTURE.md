...
- **`format1_parser.py` / `format2_parser.py` / `pt_parser.py`**：三种版式的具体解析（`scanPathBlocks`、`parseOnePath`）；`format1` classic 以行类型 + token 语义恢复字段（避免列宽漂移截断），`format2` / `pt` 从 summary 行提取 `arrival_time`、`required_time`、`slack`，PT Point 尾列下 net 的 `Fanout/Cap` 从 point 前 metric 区提取。

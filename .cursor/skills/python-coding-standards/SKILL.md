...
- **解析链路**：报告文本 (.rpt) → 解析器 (`lib/parser/`，含 TimeParser 与可选 YAML 布局）→ 结构化数据。解析器只做解析与归一化，不写文件。
- **抽取与对比**：`lib/extract.py`（及可选 `lib/parser/parallel_extract.py` 多进程）调用解析器、写 CSV；`lib/compare_path_summary.py` 与 `lib/compare/` 负责对比与报告。编排层保持 I/O，解析层无副作用。
...
- **职责单一文件**：单文件内避免堆积多种无关职责；格式差异（如 format1 / format2 / pt）放在子类或策略模块中，公共逻辑放在基类或 `lib/` 下的公共模块（如 `parser/time_parser_base.py`、`report_gen/base.py`）。
